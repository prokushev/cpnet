  ...
  for (;;) {
...

    {
      ...
      if (fnc <= 40)
        ...
        fname = "";
      goto_xy(75,3);
      printf("f:%d/%02Xh", fnc, (unsigned char) fnc);
      dump_data(buf, len, 5);
    }
    ...
    switch (fnc) {
      ...
      case 15: /* open file */
        if (_logged_in) {
          ...
          if (_debug & DEBUG_DATA) fprintf(stderr, "%s\n", getname(&fcb));
          /* close search */
          if (dirp) {
            closedir(dirp);
            dirp = NULL;
          ...
          if (dirc == 0) {
            fd = open(getname(&fcb), O_RDWR | O_BINARY);
            if (_debug & DEBUG_DATA) fprintf(stderr, "%s\n", getname(&fcb));
            if (fd < 0) {
              if (_debug & DEBUG_DATA) fprintf(stderr, "open error\n");
              dirc = 0xff;
            } else {
              int *ip;
              if (_debug & DEBUG_DATA) fprintf(stderr, "open ok\n");
              fcb.s1 = 0x80;
              fcb.s2 = 0;
              fcb.rc = 0;
              fcb.cr = 0;
      ...
      case 22: /* create file */
        if (_logged_in) {
          ...
          if (dirc == 0) {
            fd = open(getname(&fcb), O_RDWR | O_CREAT | O_BINARY, 0644);  // O_TRUNC ?
            if (fd < 0) {
              dirc = 0xff;
            } else {
              int *ip;
